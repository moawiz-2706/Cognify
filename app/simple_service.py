...
class SimpleFlashcardService:
    ...
    def generate_flashcards_for_courses(self, course_codes: List[str], username: str = "default") -> int:
        """Generate flashcards for selected courses using Gemini API with fallback to hardcoded. 
        Uses BATCH generation (single API call) to optimize rate limits. Returns count of flashcards created."""
        count = 0
        ...
        degree_program = prefs.degree_program
        semester = prefs.current_semester
        
        # Prepare course information for batch processing
        courses_info = []
        for course_code in course_codes:
            try:
                course = get_course_by_code(course_code)
                courses_info.append({"code": course_code, "name": course.name})
            except Exception as e:
                print(f"Error getting course info for {course_code}: {e}")
        
        if not courses_info:
            print("No valid courses to process")
            return 0
        
        # STEP 1: ALWAYS load hardcoded flashcards first (guaranteed to work)
        print(f"\n=== Loading hardcoded flashcards for {len(course_codes)} courses ===")
        hardcoded_by_course = {}
        for course_code in course_codes:
            hardcoded_flashcards = get_flashcards_for_courses([course_code])
            hardcoded_by_course[course_code] = hardcoded_flashcards
            print(f"Loaded {len(hardcoded_flashcards)} hardcoded flashcards for {course_code}")
        
        # STEP 2: Try to generate additional flashcards using Gemini API in ONE batch call
        gemini_flashcards_by_course = {}
        if gemini_service and len(courses_info) > 0:
            try:
                print(f"\n=== Generating flashcards for {len(courses_info)} courses in SINGLE API call ===")
                gemini_flashcards_by_course = gemini_service.generate_flashcards_for_multiple_courses(
                    courses=courses_info,
                    degree_program=degree_program,
                    semester=semester,
                    num_flashcards_per_course=10
                )
                total_gemini = sum(len(cards) for cards in gemini_flashcards_by_course.values())
                print(f"Successfully generated {total_gemini} additional flashcards via Gemini API (1 call)")
            except Exception as e:
                print(f"Error generating batch flashcards with Gemini: {e}")
                print("Using only hardcoded flashcards")
                gemini_flashcards_by_course = {course["code"]: [] for course in courses_info}
        else:
            print("Gemini service not available, using only hardcoded flashcards")
            gemini_flashcards_by_course = {course["code"]: [] for course in courses_info}
        
        # STEP 3: Combine hardcoded + Gemini flashcards and create Flashcard objects
        print(f"\n=== Creating flashcards for {len(course_codes)} courses ===")
        for course_code in course_codes:
            try:
                # Combine hardcoded and Gemini flashcards
                flashcards_data = hardcoded_by_course.get(course_code, [])
                gemini_flashcards = gemini_flashcards_by_course.get(course_code, [])
                
                # Add Gemini flashcards
                flashcards_data.extend(gemini_flashcards)
                
                print(f"Processing {course_code}: {len(hardcoded_by_course.get(course_code, []))} hardcoded + {len(gemini_flashcards)} AI-generated = {len(flashcards_data)} total")
                
                # Create flashcards from generated data
                course_count = 0  # Initialize course_count
                ...
                print(f"  Created {course_count} new flashcards for {course_code}")
                ...
                try:
                    ...
                    print(f"Critical error with fallback flashcards for {course_code}: {fallback_error}")
        
        # Final update of next_id
        user_data["next_id"] = next_id
        total_flashcards = len(flashcards)
        print(f"\n=== SUMMARY ===")
        print(f"Created {count} new flashcards for user {username}")
        print(f"Total flashcards for {username}: {total_flashcards}")
        print(f"API calls used: 1 (batch generation for {len(course_codes)} courses)")
        return count
    ...
    def add_course_to_semester(self, course_code: str, username: str = "default") -> bool:
        ...
        if course_code not in prefs.selected_courses:
            ...
            try:
                ...
                print(f"Generated {count} flashcards for course {course_code} for user {username}")
                if count == 0:
                    print(f"WARNING: No flashcards generated for {course_code}. Check Gemini API or fallback.")
            except Exception as e:
